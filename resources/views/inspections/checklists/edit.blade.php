    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex justify-between items-center py-4">
            <div class="flex items-center space-x-3">
                <a href="{{ route('inspections.checklists.show', $checklist) }}" class="text-gray-500 hover:text-black">
                    <i class="fas fa-arrow-left mr-2"></i>Back
                </a>
                <h1 class="text-2xl font-bold text-black">Edit Inspection Checklist</h1>
            </div>
        </div>
    </div>
</div>
...
<div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
    <form action="{{ route('inspections.checklists.update', $checklist) }}" method="POST" class="space-y-6">
        @csrf
        @method('PUT')
        ...
        <div class="bg-white border border-gray-300 p-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <input type="text" id="name" name="name" required value="{{ old('name', $checklist->name) }}"
                ...
                <div>
                    ...
                    <input type="text" id="category" name="category" value="{{ old('category', $checklist->category) }}"
                           class="w-full px-3 py-2 border border-gray-300 focus:border-[#0066CC] focus:ring-1 focus:ring-[#0066CC]">
                </div>
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea id="description" name="description" rows="3"
                              class="w-full px-3 py-2 border border-gray-300 focus:border-[#0066CC] focus:ring-1 focus:ring-[#0066CC]">{{ old('description', $checklist->description) }}</textarea>
                </div>

                <div>
                    <label class="flex items-center">
                        <input type="checkbox" name="is_active" value="1" {{ old('is_active', $checklist->is_active) ? 'checked' : '' }}
                    ...
                    <div class="checklist-item border border-gray-300 p-4">
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-3">
                            <div>
                                ...
                                <input type="text" name="items[{{ $index }}][item]" required value="{{ $item['item'] ?? '' }}"
                                       class="w-full px-3 py-2 border border-gray-300 focus:border-[#0066CC] focus:ring-1 focus:ring-[#0066CC]">
                            </div>
                            <div>
        ...
        <div class="flex justify-end space-x-3">
            <a href="{{ route('inspections.checklists.show', $checklist) }}" class="px-6 py-2 border border-gray-300 hover:bg-[#F5F5F5]">Cancel</a>
...
@push('scripts')
<script>
let itemIndex = {{ count($checklist->items ?? []) }};
