    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex justify-between items-center py-4">
            <div class="flex items-center space-x-3">
                <a href="{{ route('inspections.audit-findings.show', $finding) }}" class="text-gray-500 hover:text-black">
                    <i class="fas fa-arrow-left mr-2"></i>Back
                </a>
                <h1 class="text-2xl font-bold text-black">Edit Audit Finding</h1>
            </div>
        </div>
    </div>
</div>
...
<div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
    <form action="{{ route('inspections.audit-findings.update', $finding) }}" method="POST" class="space-y-6">
        @csrf
        @method('PUT')
        ...
        <div class="bg-white border border-gray-300 p-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div class="md:col-span-2">
                    ...
                    <input type="text" id="title" name="title" required value="{{ old('title', $finding->title) }}"
                ...
                <div>
                    ...
                    <select id="finding_type" name="finding_type" required
                            class="w-full px-3 py-2 border border-gray-300 focus:border-[#0066CC] focus:ring-1 focus:ring-[#0066CC]">
                        <option value="non_conformance" {{ old('finding_type', $finding->finding_type) == 'non_conformance' ? 'selected' : '' }}>Non-Conformance</option>
                        <option value="observation" {{ old('finding_type', $finding->finding_type) == 'observation' ? 'selected' : '' }}>Observation</option>
                        <option value="opportunity_for_improvement" {{ old('finding_type', $finding->finding_type) == 'opportunity_for_improvement' ? 'selected' : '' }}>Opportunity for Improvement</option>
                        <option value="strength" {{ old('finding_type', $finding->finding_type) == 'strength' ? 'selected' : '' }}>Strength</option>
                    </select>
                    @error('finding_type')
                        <p class="mt-1 text-sm text-[#CC0000]">{{ $message }}</p>
                    @enderror
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea id="description" name="description" rows="4" required
                              class="w-full px-3 py-2 border border-gray-300 focus:border-[#0066CC] focus:ring-1 focus:ring-[#0066CC]">{{ old('description', $finding->description) }}</textarea>
                    @error('description')
                        <p class="mt-1 text-sm text-[#CC0000]">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <input type="text" id="clause_reference" name="clause_reference" value="{{ old('clause_reference', $finding->clause_reference) }}"
                           class="w-full px-3 py-2 border border-gray-300 focus:border-[#0066CC] focus:ring-1 focus:ring-[#0066CC]">
                </div>

                <div>
                    <label for="evidence" class="block text-sm font-medium text-black mb-1">Evidence</label>
                    <input type="text" id="evidence" name="evidence" value="{{ old('evidence', $finding->evidence) }}"
                           class="w-full px-3 py-2 border border-gray-300 focus:border-[#0066CC] focus:ring-1 focus:ring-[#0066CC]">
                </div>
            </div>
        </div>
            ...
            <div class="space-y-4">
                <div>
                    ...
                    <textarea id="root_cause" name="root_cause" rows="3"
                              class="w-full px-3 py-2 border border-gray-300 focus:border-[#0066CC] focus:ring-1 focus:ring-[#0066CC]">{{ old('root_cause', $finding->root_cause) }}</textarea>
                </div>

                <div>
                    <label class="flex items-center">
                        <input type="checkbox" name="corrective_action_required" value="1" {{ old('corrective_action_required', $finding->corrective_action_required) ? 'checked' : '' }}
                ...
                <div id="corrective-action-fields" class="space-y-4" style="display: {{ old('corrective_action_required', $finding->corrective_action_required) ? 'block' : 'none' }};">
                    <div>
                        ...
                        <textarea id="corrective_action_plan" name="corrective_action_plan" rows="3"
                                  class="w-full px-3 py-2 border border-gray-300 focus:border-[#0066CC] focus:ring-1 focus:ring-[#0066CC]">{{ old('corrective_action_plan', $finding->corrective_action_plan) }}</textarea>
                    </div>

                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            <label for="corrective_action_due_date" class="block text-sm font-medium text-black mb-1">Due Date</label>
                            <input type="date" id="corrective_action_due_date" name="corrective_action_due_date" value="{{ old('corrective_action_due_date', $finding->corrective_action_due_date?->format('Y-m-d')) }}"
                                   class="w-full px-3 py-2 border border-gray-300 focus:border-[#0066CC] focus:ring-1 focus:ring-[#0066CC]">
                        </div>
                        ...
                        <div>
                            ...
                            <select id="corrective_action_assigned_to" name="corrective_action_assigned_to"
                                ...
                                @foreach($users as $user)
                                    <option value="{{ $user->id }}" {{ old('corrective_action_assigned_to', $finding->corrective_action_assigned_to) == $user->id ? 'selected' : '' }}>
                                        {{ $user->name }}
                                    </option>
                                @endforeach
                            </select>
                    ...
                    <div>
                        <label class="flex items-center">
                            <input type="checkbox" name="corrective_action_completed" value="1" {{ old('corrective_action_completed', $finding->corrective_action_completed) ? 'checked' : '' }}
                                   class="h-4 w-4 text-[#0066CC] focus:ring-[#0066CC] border-gray-300">
                            <span class="ml-2 text-sm text-black">Corrective Action Completed</span>
                        </label>
                    </div>

                    @if($finding->corrective_action_completed)
                    <div>
                        <label for="corrective_action_completed_date" class="block text-sm font-medium text-black mb-1">Completed Date</label>
                        <input type="date" id="corrective_action_completed_date" name="corrective_action_completed_date" value="{{ old('corrective_action_completed_date', $finding->corrective_action_completed_date?->format('Y-m-d')) }}"
                               class="w-full px-3 py-2 border border-gray-300 focus:border-[#0066CC] focus:ring-1 focus:ring-[#0066CC]">
                    </div>
                    @endif
                </div>
        ...
        <div class="bg-white border border-gray-300 p-6">
            ...
            <div class="space-y-4">
                <div>
                    <label for="verified_by" class="block text-sm font-medium text-black mb-1">Verified By</label>
                    <select id="verified_by" name="verified_by"
                            class="w-full px-3 py-2 border border-gray-300 focus:border-[#0066CC] focus:ring-1 focus:ring-[#0066CC]">
                        <option value="">Select User</option>
                        @foreach($users as $user)
                            <option value="{{ $user->id }}" {{ old('verified_by', $finding->verified_by) == $user->id ? 'selected' : '' }}>
                                {{ $user->name }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div>
                    <label for="verified_at" class="block text-sm font-medium text-black mb-1">Verified At</label>
                    <input type="date" id="verified_at" name="verified_at" value="{{ old('verified_at', $finding->verified_at?->format('Y-m-d')) }}"
                           class="w-full px-3 py-2 border border-gray-300 focus:border-[#0066CC] focus:ring-1 focus:ring-[#0066CC]">
                </div>
                ...
                <div>
                    ...
                    <textarea id="verification_notes" name="verification_notes" rows="3"
                              class="w-full px-3 py-2 border border-gray-300 focus:border-[#0066CC] focus:ring-1 focus:ring-[#0066CC]">{{ old('verification_notes', $finding->verification_notes) }}</textarea>
                </div>
            </div>
        </div>
        ...
        <div class="flex justify-end space-x-3">
            <a href="{{ route('inspections.audit-findings.show', $finding) }}" class="px-6 py-2 border border-gray-300 hover:bg-[#F5F5F5]">Cancel</a>
