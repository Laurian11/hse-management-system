    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex justify-between items-center py-4">
            <div class="flex items-center space-x-3">
                <a href="{{ route('inspections.schedules.show', $schedule) }}" class="text-gray-500 hover:text-black">
                    <i class="fas fa-arrow-left mr-2"></i>Back
                </a>
                <h1 class="text-2xl font-bold text-black">Edit Inspection Schedule</h1>
            </div>
        </div>
    </div>
</div>
...
<div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
    <form action="{{ route('inspections.schedules.update', $schedule) }}" method="POST" class="space-y-6">
        @csrf
        @method('PUT')
        ...
        <div class="bg-white border border-gray-300 p-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div class="md:col-span-2">
                    ...
                    <input type="text" id="title" name="title" required value="{{ old('title', $schedule->title) }}"
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea id="description" name="description" rows="3"
                              class="w-full px-3 py-2 border border-gray-300 focus:border-[#0066CC] focus:ring-1 focus:ring-[#0066CC]">{{ old('description', $schedule->description) }}</textarea>
                </div>
                ...
                <div>
                    ...
                    <select id="frequency" name="frequency" required
                        ...
                        <option value="annually" {{ old('frequency', $schedule->frequency) == 'annually' ? 'selected' : '' }}>Annually</option>
                ...
                <div id="custom-days-field" style="display: {{ old('frequency', $schedule->frequency) == 'custom' ? 'block' : 'none' }};">
                    <label for="custom_days" class="block text-sm font-medium text-black mb-1">Custom Days *</label>
                    <input type="number" id="custom_days" name="custom_days" min="1" value="{{ old('custom_days', $schedule->custom_days) }}"
                           class="w-full px-3 py-2 border border-gray-300 focus:border-[#0066CC] focus:ring-1 focus:ring-[#0066CC]">
                    @error('custom_days')
                        <p class="mt-1 text-sm text-[#CC0000]">{{ $message }}</p>
                    @enderror
                ...
                <div>
                    ...
                    <input type="date" id="start_date" name="start_date" required value="{{ old('start_date', $schedule->start_date->format('Y-m-d')) }}"
                           class="w-full px-3 py-2 border border-gray-300 focus:border-[#0066CC] focus:ring-1 focus:ring-[#0066CC]">
                    @error('start_date')
                        <p class="mt-1 text-sm text-[#CC0000]">{{ $message }}</p>
                    @enderror
                ...
                <div>
                    ...
                    <input type="date" id="end_date" name="end_date" value="{{ old('end_date', $schedule->end_date?->format('Y-m-d')) }}"
                           class="w-full px-3 py-2 border border-gray-300 focus:border-[#0066CC] focus:ring-1 focus:ring-[#0066CC]">
                </div>

                <div>
                    <label for="next_inspection_date" class="block text-sm font-medium text-black mb-1">Next Inspection Date *</label>
                    <input type="date" id="next_inspection_date" name="next_inspection_date" required value="{{ old('next_inspection_date', $schedule->next_inspection_date->format('Y-m-d')) }}"
                           class="w-full px-3 py-2 border border-gray-300 focus:border-[#0066CC] focus:ring-1 focus:ring-[#0066CC]">
                ...
                <div>
                    ...
                    <select id="inspection_checklist_id" name="inspection_checklist_id"
                        ...
                        @foreach($checklists as $checklist)
                            <option value="{{ $checklist->id }}" {{ old('inspection_checklist_id', $schedule->inspection_checklist_id) == $checklist->id ? 'selected' : '' }}>
                                {{ $checklist->name }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea id="notes" name="notes" rows="3"
                              class="w-full px-3 py-2 border border-gray-300 focus:border-[#0066CC] focus:ring-1 focus:ring-[#0066CC]">{{ old('notes', $schedule->notes) }}</textarea>
                </div>
        ...
        <div class="flex justify-end space-x-3">
            <a href="{{ route('inspections.schedules.show', $schedule) }}" class="px-6 py-2 border border-gray-300 hover:bg-[#F5F5F5]">Cancel</a>
