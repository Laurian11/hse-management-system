...
@section('title', 'Create Toolbox Talk')
...
<div class="min-h-screen bg-gray-50">
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="flex justify-between items-center py-4">
                <div class="flex items-center space-x-3">
                    ...
                    <h1 class="text-2xl font-bold text-gray-900">Create Toolbox Talk</h1>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <form action="{{ route('toolbox-talks.store') }}" method="POST" class="space-y-6">
            @csrf
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div class="md:col-span-2">
                        ...
                        <textarea id="description" name="description" rows="3"
                                  class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                                  placeholder="Enter talk description"></textarea>
                        @error('description')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
            ...
            <div class="bg-white rounded-lg shadow p-6">
                ...
                <div class="space-y-4">
                    <div class="flex items-center">
                        <input type="checkbox" id="biometric_required" name="biometric_required" value="1" checked
                    ...
                    <div class="flex items-center">
                        <input type="checkbox" id="is_recurring" name="is_recurring" value="1"
                               class="h-4 w-4 text-blue-600 focus:ring-blue-500 border-gray-300 rounded">
                        <label for="is_recurring" class="ml-3 text-sm text-gray-700">
                            Make this a recurring talk
                        </label>
                    </div>

                    <div id="recurrence_options" class="hidden ml-7">
            ...
            <div class="bg-white rounded-lg shadow p-6">
                <div class="flex justify-end space-x-4">
                    ...
                    <button type="submit" 
                        ...
                        <i class="fas fa-save mr-2"></i>Create Talk
                    </button>
                </div>
            </div>
        </form>
...
<script>
    ...
    document.querySelector('form').addEventListener('submit', function(e) {
        const biometricRequired = document.getElementById('biometric_required').checked;
        const isRecurring = document.getElementById('is_recurring').checked;
        
        if (isRecurring && !document.getElementById('recurrence_pattern').value) {
            e.preventDefault();
            alert('Please select a recurrence pattern for recurring talks.');
            return;
        }
    });
</script>
