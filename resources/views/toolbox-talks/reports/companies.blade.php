...
<div class="min-h-screen bg-gray-50">
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="flex justify-between items-center py-4">
                <div class="flex items-center space-x-3">
                    ...
                    <h1 class="text-2xl font-bold text-gray-900">Companies Report</h1>
                </div>
                <div class="flex space-x-3">
                    <a href="{{ route('toolbox-talks.reports.companies', array_merge(request()->all(), ['format' => 'excel'])) }}" 
                       class="px-4 py-2 text-green-700 bg-white border border-green-300 rounded-lg hover:bg-green-50">
                        <i class="fas fa-file-excel mr-2"></i>Export Excel
                    </a>
                    <a href="{{ route('toolbox-talks.reports.companies', array_merge(request()->all(), ['format' => 'pdf'])) }}" 
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div class="bg-white rounded-lg shadow p-6 mb-6">
            <form method="GET" action="{{ route('toolbox-talks.reports.companies') }}" class="grid grid-cols-1 md:grid-cols-4 gap-4">
                <div>
                ...
                <div class="flex items-end">
                    <a href="{{ route('toolbox-talks.reports.companies') }}" class="w-full px-4 py-2 text-gray-700 bg-white border border-gray-300 rounded-lg hover:bg-gray-50 text-center">
        ...
        <div class="bg-white rounded-lg shadow overflow-hidden">
            <div class="px-6 py-4 border-b border-gray-200">
                <h2 class="text-lg font-semibold text-gray-900">Company Performance Comparison</h2>
            </div>
                ...
                <table class="min-w-full divide-y divide-gray-200">
                    <thead class="bg-gray-50">
                        <tr>
                            ...
                            <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Avg Feedback</th>
                        </tr>
                    </thead>
                    <tbody class="bg-white divide-y divide-gray-200">
                        @forelse($companyStats as $comp)
                            <tr>
                                ...
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="text-sm text-gray-900">{{ $comp['total_talks'] }}</div>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="text-sm text-gray-900">{{ $comp['completed_talks'] }}</div>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="flex items-center">
                                        <div class="flex-1 bg-gray-200 rounded-full h-4 mr-2">
                                            <div class="bg-green-600 h-4 rounded-full flex items-center justify-end pr-2" 
                                                 style="width: {{ min(100, $comp['completion_rate']) }}%">
                                                @if($comp['completion_rate'] > 10)
                                                    <span class="text-xs text-white font-medium">{{ number_format($comp['completion_rate'], 1) }}%</span>
                                                @endif
                                            </div>
                                        </div>
                                        <span class="text-sm font-medium text-gray-900">{{ number_format($comp['completion_rate'], 2) }}%</span>
                                    </div>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="text-sm text-gray-900">{{ $comp['total_attendees'] }}</div>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="text-sm text-gray-900">{{ $comp['present_attendees'] }}</div>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="flex items-center">
                                        <div class="flex-1 bg-gray-200 rounded-full h-4 mr-2">
                                            <div class="bg-blue-600 h-4 rounded-full flex items-center justify-end pr-2" 
                                                 style="width: {{ min(100, $comp['attendance_rate']) }}%">
                                                @if($comp['attendance_rate'] > 10)
                                                    <span class="text-xs text-white font-medium">{{ number_format($comp['attendance_rate'], 1) }}%</span>
                                                @endif
                                            </div>
                                        </div>
                                        <span class="text-sm font-medium text-gray-900">{{ number_format($comp['attendance_rate'], 2) }}%</span>
                                    </div>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="text-sm text-gray-900">{{ number_format($comp['avg_feedback'], 2) }}/5</div>
                                </td>
                            </tr>
                        @empty
                            <tr>
