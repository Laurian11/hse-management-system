...
@section('title', 'Create Job Safety Analysis')
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex justify-between items-center py-4">
            <div class="flex items-center space-x-3">
                <a href="{{ route('risk-assessment.jsas.index') }}" class="text-gray-600 hover:text-gray-700">
                    <i class="fas fa-arrow-left mr-2"></i>Back to JSAs
                </a>
                <h1 class="text-2xl font-bold text-gray-900">Create Job Safety Analysis (JSA)</h1>
            </div>
        </div>
    </div>
</div>
...
<div class="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
    <form action="{{ route('risk-assessment.jsas.store') }}" method="POST" class="space-y-6" id="jsaForm">
        @csrf
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea id="job_description" name="job_description" rows="4" required
                              class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-yellow-500 focus:border-yellow-500"
                              placeholder="Detailed description of the job/task">{{ old('job_description') }}</textarea>
                ...
                <div>
                    ...
                    <input type="text" id="work_area" name="work_area" value="{{ old('work_area') }}"
                           class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-yellow-500 focus:border-yellow-500"
                ...
                <div>
                    ...
                    <input type="date" id="job_date" name="job_date" value="{{ old('job_date') }}"
                           class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-yellow-500 focus:border-yellow-500">
                </div>
                ...
                <div class="md:col-span-2">
                    ...
                    <select id="related_risk_assessment_id" name="related_risk_assessment_id"
                        ...
                        @foreach($riskAssessments as $assessment)
                            <option value="{{ $assessment->id }}" {{ old('related_risk_assessment_id', $selectedRiskAssessment?->id) == $assessment->id ? 'selected' : '' }}>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <textarea id="required_qualifications" name="required_qualifications" rows="3"
                              class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-yellow-500 focus:border-yellow-500"
                              placeholder="e.g., Certified Welder, Confined Space Entry Permit">{{ old('required_qualifications') }}</textarea>
                </div>
                ...
                <div>
                    ...
                    <textarea id="required_training" name="required_training" rows="3"
                              class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-yellow-500 focus:border-yellow-500"
                              placeholder="e.g., Lockout/Tagout, Fall Protection">{{ old('required_training') }}</textarea>
                </div>
                ...
                <div>
                    ...
                    <div id="materialsContainer" class="space-y-2">
                        <!-- Materials items will be added here dynamically -->
                    </div>
                ...
                <div class="md:col-span-2">
                    ...
                    <div id="ppeContainer" class="space-y-2">
                        <!-- PPE items will be added here dynamically -->
                    </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <textarea id="weather_conditions" name="weather_conditions" rows="3"
                              class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-yellow-500 focus:border-yellow-500"
                              placeholder="e.g., Clear, 25°C, Light wind">{{ old('weather_conditions') }}</textarea>
                </div>
                ...
                <div>
                    ...
                    <textarea id="site_conditions" name="site_conditions" rows="3"
                              class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-yellow-500 focus:border-yellow-500"
                              placeholder="e.g., Dry, Well-lit, Adequate ventilation">{{ old('site_conditions') }}</textarea>
                </div>
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea id="special_considerations" name="special_considerations" rows="3"
                              class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-yellow-500 focus:border-yellow-500"
                              placeholder="Any special considerations or precautions">{{ old('special_considerations') }}</textarea>
                </div>
            </div>
        </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <textarea id="emergency_contacts" name="emergency_contacts" rows="3"
                              class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-yellow-500 focus:border-yellow-500"
                              placeholder="Emergency contact numbers and names">{{ old('emergency_contacts') }}</textarea>
                </div>
                ...
                <div>
                    ...
                    <input type="text" id="first_aid_location" name="first_aid_location" value="{{ old('first_aid_location') }}"
                           class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-yellow-500 focus:border-yellow-500"
                           placeholder="Location of first aid kit/station">
                </div>
                ...
                <div>
                    ...
                    <textarea id="emergency_procedures" name="emergency_procedures" rows="3"
                              class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-yellow-500 focus:border-yellow-500"
                              placeholder="Emergency response procedures">{{ old('emergency_procedures') }}</textarea>
                </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <select id="overall_risk_level" name="overall_risk_level" required
                        ...
                        <option value="low" {{ old('overall_risk_level') == 'low' ? 'selected' : '' }}>Low</option>
                        <option value="medium" {{ old('overall_risk_level') == 'medium' ? 'selected' : '' }}>Medium</option>
                        <option value="high" {{ old('overall_risk_level') == 'high' ? 'selected' : '' }}>High</option>
                        <option value="critical" {{ old('overall_risk_level') == 'critical' ? 'selected' : '' }}>Critical</option>
                    </select>
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea id="risk_summary" name="risk_summary" rows="4"
                              class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-yellow-500 focus:border-yellow-500"
                              placeholder="Summary of overall risks and key control measures">{{ old('risk_summary') }}</textarea>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white rounded-lg shadow p-6">
            <div class="flex justify-end space-x-4">
                ...
                <button type="submit" 
                    ...
                    <i class="fas fa-save mr-2"></i>Create JSA
                </button>
            </div>
        </div>
    </form>
