    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex justify-between items-center py-4">
            <div class="flex items-center space-x-3">
                ...
                <h1 class="text-2xl font-bold text-black">Create Compliance Requirement</h1>
            </div>
        </div>
    </div>
</div>
...
<div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
    <form action="{{ route('compliance.requirements.store') }}" method="POST" class="space-y-6">
        @csrf
        ...
        <div class="bg-white border border-gray-300 p-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <input type="text" id="requirement_title" name="requirement_title" required value="{{ old('requirement_title') }}"
                ...
                <div>
                    ...
                    <input type="text" id="regulatory_body" name="regulatory_body" required value="{{ old('regulatory_body') }}"
                           placeholder="e.g., GCLA, OSHA, NEMC"
                           class="w-full px-3 py-2 border border-gray-300 focus:border-[#0066CC] focus:ring-1 focus:ring-[#0066CC]">
                ...
                <div>
                    <label for="regulation_code" class="block text-sm font-medium text-black mb-1">Regulation Code</label>
                    <input type="text" id="regulation_code" name="regulation_code" value="{{ old('regulation_code') }}"
                           placeholder="e.g., OSHA 1910.147"
                           class="w-full px-3 py-2 border border-gray-300 focus:border-[#0066CC] focus:ring-1 focus:ring-[#0066CC]">
                </div>

                <div>
                    <label for="requirement_type" class="block text-sm font-medium text-black mb-1">Requirement Type *</label>
                    <select id="requirement_type" name="requirement_type" required
                            class="w-full px-3 py-2 border border-gray-300 focus:border-[#0066CC] focus:ring-1 focus:ring-[#0066CC]">
                        <option value="">Select Type</option>
                        <option value="regulatory" {{ old('requirement_type') == 'regulatory' ? 'selected' : '' }}>Regulatory</option>
                        <option value="internal_policy" {{ old('requirement_type') == 'internal_policy' ? 'selected' : '' }}>Internal Policy</option>
                        <option value="standard" {{ old('requirement_type') == 'standard' ? 'selected' : '' }}>Standard</option>
                ...
                <div>
                    ...
                    <select id="compliance_status" name="compliance_status" required
                        ...
                        <option value="partially_compliant" {{ old('compliance_status') == 'partially_compliant' ? 'selected' : '' }}>Partially Compliant</option>
                        <option value="not_applicable" {{ old('compliance_status') == 'not_applicable' ? 'selected' : '' }}>Not Applicable</option>
                        <option value="under_review" {{ old('compliance_status') == 'under_review' ? 'selected' : '' }}>Under Review</option>
                    </select>
                ...
                <div>
                    ...
                    <input type="date" id="compliance_due_date" name="compliance_due_date" value="{{ old('compliance_due_date') }}"
                           class="w-full px-3 py-2 border border-gray-300 focus:border-[#0066CC] focus:ring-1 focus:ring-[#0066CC]">
                </div>
                ...
                <div class="md:col-span-2">
                    <label for="compliance_evidence" class="block text-sm font-medium text-black mb-1">Compliance Evidence</label>
                    <textarea id="compliance_evidence" name="compliance_evidence" rows="3"
                              class="w-full px-3 py-2 border border-gray-300 focus:border-[#0066CC] focus:ring-1 focus:ring-[#0066CC]">{{ old('compliance_evidence') }}</textarea>
                </div>
        ...
        <div class="flex justify-end space-x-3">
            <a href="{{ route('compliance.requirements.index') }}" class="px-6 py-2 border border-gray-300 hover:bg-[#F5F5F5]">Cancel</a>
