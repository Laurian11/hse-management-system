    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex justify-between items-center py-4">
            <div class="flex items-center space-x-3">
                <a href="{{ route('admin.employees.index') }}" class="text-gray-600 hover:text-gray-700">
...
<div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
    <form action="{{ route('admin.employees.update', $employee) }}" method="POST" class="space-y-6">
        ...
        <div class="bg-white rounded-lg shadow p-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <input type="text" id="name" name="name" required value="{{ old('name', $employee->name) }}"
        ...
        <div class="bg-white rounded-lg shadow p-6">
            <h2 class="text-lg font-semibold text-gray-900 mb-4">Role & Department Assignment</h2>
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                ...
                <div>
                    ...
                    <select id="direct_supervisor_id" name="direct_supervisor_id"
                        ...
                        @foreach($supervisors as $supervisor)
                            <option value="{{ $supervisor->id }}" {{ old('direct_supervisor_id', $employee->direct_supervisor_id) == $supervisor->id ? 'selected' : '' }}>
                                {{ $supervisor->name }} ({{ $supervisor->job_title ?? 'N/A' }})
                            </option>
                        @endforeach
                    </select>
        ...
        <div class="bg-white rounded-lg shadow p-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <input type="text" id="employee_id_number" name="employee_id_number" value="{{ old('employee_id_number', $employee->employee_id_number) }}"
                           class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
                ...
                <div>
                    ...
                    <select id="employment_type" name="employment_type" required
                        ...
                        <option value="contractor" {{ old('employment_type', $employee->employment_type) == 'contractor' ? 'selected' : '' }}>Contractor</option>
                        <option value="visitor" {{ old('employment_type', $employee->employment_type) == 'visitor' ? 'selected' : '' }}>Visitor</option>
                    </select>
                ...
                <div>
                    ...
                    <input type="date" id="date_of_hire" name="date_of_hire" value="{{ old('date_of_hire', $employee->date_of_hire ? $employee->date_of_hire->format('Y-m-d') : '') }}"
                           class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
                    @error('date_of_hire')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
