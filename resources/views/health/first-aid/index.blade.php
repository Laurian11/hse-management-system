...
@section('title', 'First Aid Logbook')
...
<div class="bg-white border-b border-gray-300">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex justify-between items-center py-4">
            <div>
                <h1 class="text-2xl font-bold text-black">First Aid Logbook</h1>
                <p class="text-sm text-gray-500 mt-1">First aid incident records</p>
            </div>
            <div>
                <a href="{{ route('health.dashboard') }}" class="text-[#0066CC] hover:underline mr-4">Dashboard</a>
                <a href="{{ route('health.first-aid.create') }}" class="bg-[#0066CC] text-white px-4 py-2 border border-[#0066CC] hover:bg-[#0052A3]">
    ...
    <div class="bg-white border border-gray-300 p-4 mb-6">
        <form method="GET" class="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div class="flex space-x-2">
                ...
                <a href="{{ route('health.first-aid.index') }}" class="px-4 py-2 border border-gray-300 hover:bg-[#F5F5F5]">Reset</a>
            </div>
        </form>
    </div>
        ...
        <table class="min-w-full divide-y divide-gray-300">
            <thead class="bg-[#F5F5F5]">
                <tr>
                    ...
                    <th class="px-6 py-3 text-left text-xs font-medium text-black uppercase">Injured Person</th>
            ...
            <tbody class="bg-white divide-y divide-gray-300">
                @forelse($entries as $entry)
                    <tr class="hover:bg-[#F5F5F5]">
                        <td class="px-6 py-4 text-sm font-medium text-black">{{ $entry->reference_number }}</td>
                        <td class="px-6 py-4 text-sm text-black">{{ $entry->incident_date->format('M d, Y') }}</td>
                        <td class="px-6 py-4 text-sm text-black">{{ $entry->injured_person_name ?? ($entry->injuredPerson->name ?? 'N/A') }}</td>
                        <td class="px-6 py-4 text-sm text-black">{{ $entry->location ?? 'N/A' }}</td>
                        <td class="px-6 py-4">
                        ...
                        <td class="px-6 py-4 text-right text-sm">
                            <a href="{{ route('health.first-aid.show', $entry) }}" class="text-[#0066CC] hover:underline mr-3">View</a>
                            <a href="{{ route('health.first-aid.edit', $entry) }}" class="text-[#0066CC] hover:underline">Edit</a>
                        </td>
                    </tr>
                @empty
