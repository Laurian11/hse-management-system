...
@section('title', 'Workplace Hygiene Inspections')
...
<div class="bg-white border-b border-gray-300">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex justify-between items-center py-4">
            <div>
                <h1 class="text-2xl font-bold text-black">Workplace Hygiene Inspections</h1>
                <p class="text-sm text-gray-500 mt-1">Hygiene inspection records</p>
            </div>
            <div>
                <a href="{{ route('health.dashboard') }}" class="text-[#0066CC] hover:underline mr-4">Dashboard</a>
                <a href="{{ route('health.hygiene.create') }}" class="bg-[#0066CC] text-white px-4 py-2 border border-[#0066CC] hover:bg-[#0052A3]">
    ...
    <div class="bg-white border border-gray-300 p-4 mb-6">
        <form method="GET" class="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <select name="overall_status" class="px-3 py-2 border border-gray-300 focus:border-[#0066CC] focus:ring-1 focus:ring-[#0066CC]">
                ...
                <option value="satisfactory" {{ request('overall_status') == 'satisfactory' ? 'selected' : '' }}>Satisfactory</option>
                <option value="needs_improvement" {{ request('overall_status') == 'needs_improvement' ? 'selected' : '' }}>Needs Improvement</option>
            </select>
            <div class="flex space-x-2">
                <button type="submit" class="flex-1 bg-[#0066CC] text-white px-4 py-2 border border-[#0066CC] hover:bg-[#0052A3]">Filter</button>
                <a href="{{ route('health.hygiene.index') }}" class="px-4 py-2 border border-gray-300 hover:bg-[#F5F5F5]">Reset</a>
            </div>
        </form>
    </div>
            ...
            <tbody class="bg-white divide-y divide-gray-300">
                @forelse($inspections as $inspection)
                    <tr class="hover:bg-[#F5F5F5]">
                        ...
                        <td class="px-6 py-4 text-sm text-black">{{ $inspection->area_inspected ?? 'N/A' }}</td>
                        <td class="px-6 py-4">
                            <span class="px-2 py-1 text-xs font-semibold border border-gray-300
                                {{ $inspection->overall_status == 'satisfactory' ? 'bg-[#F5F5F5] text-[#0066CC] border-[#0066CC]' : 'bg-[#F5F5F5] text-[#FF9900] border-[#FF9900]' }}">
                                {{ ucfirst(str_replace('_', ' ', $inspection->overall_status)) }}
                            </span>
                        </td>
                        <td class="px-6 py-4 text-right text-sm">
                            <a href="{{ route('health.hygiene.show', $inspection) }}" class="text-[#0066CC] hover:underline mr-3">View</a>
                            <a href="{{ route('health.hygiene.edit', $inspection) }}" class="text-[#0066CC] hover:underline">Edit</a>
                        </td>
                    </tr>
                @empty
                    <tr><td colspan="5" class="px-6 py-4 text-center text-gray-500">No inspections found.</td></tr>
                @endforelse
            </tbody>
        </table>
    </div>
