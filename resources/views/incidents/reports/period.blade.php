...
@section('title', 'Period Incident Report')
...
<div class="min-h-screen bg-gray-50">
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="flex justify-between items-center py-4">
                <div class="flex items-center space-x-3">
                    ...
                    <h1 class="text-2xl font-bold text-gray-900">Period Incident Report</h1>
                </div>
                <div class="flex space-x-3">
                    <a href="{{ route('incidents.reports.period', array_merge(request()->all(), ['format' => 'excel'])) }}" 
                       class="px-4 py-2 text-green-700 bg-white border border-green-300 rounded-lg hover:bg-green-50">
                        <i class="fas fa-file-excel mr-2"></i>Export Excel
                    </a>
                    <a href="{{ route('incidents.reports.period', array_merge(request()->all(), ['format' => 'pdf'])) }}" 
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div class="bg-white rounded-lg shadow p-6 mb-6">
            <form method="GET" action="{{ route('incidents.reports.period') }}" class="grid grid-cols-1 md:grid-cols-4 gap-4">
                <div>
                ...
                <div class="flex items-end">
                    <a href="{{ route('incidents.reports.period') }}" class="w-full px-4 py-2 text-gray-700 bg-white border border-gray-300 rounded-lg hover:bg-gray-50 text-center">
            ...
            <div class="bg-white rounded-lg shadow p-6">
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <p class="text-2xl font-bold text-gray-900 mt-1">{{ $stats['total_incidents'] }}</p>
                    </div>
            ...
            <div class="bg-white rounded-lg shadow p-6">
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <p class="text-2xl font-bold text-yellow-600 mt-1">{{ $stats['investigating'] }}</p>
                    </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
            <div class="bg-white rounded-lg shadow p-6">
                <h3 class="text-lg font-semibold text-gray-900 mb-4">Severity Breakdown</h3>
                <div class="space-y-3">
                    <div class="flex items-center justify-between">
            ...
            <div class="bg-white rounded-lg shadow p-6">
                ...
                <div class="space-y-3">
                    <div class="flex items-center justify-between">
                        <span class="text-sm text-gray-600">Injury/Illness</span>
                        <span class="px-3 py-1 bg-red-100 text-red-800 rounded-full text-sm font-semibold">{{ $stats['injury_illness'] }}</span>
                    </div>
                    <div class="flex items-center justify-between">
                        <span class="text-sm text-gray-600">Property Damage</span>
                        <span class="px-3 py-1 bg-orange-100 text-orange-800 rounded-full text-sm font-semibold">{{ $stats['property_damage'] }}</span>
                    </div>
                    <div class="flex items-center justify-between">
                        <span class="text-sm text-gray-600">Near Miss</span>
                        <span class="px-3 py-1 bg-yellow-100 text-yellow-800 rounded-full text-sm font-semibold">{{ $stats['near_miss'] }}</span>
                    </div>
                </div>
            </div>
        </div>
                    ...
                    <tbody class="bg-white divide-y divide-gray-200">
                        @forelse($incidents as $incident)
                            <tr class="hover:bg-gray-50">
                                ...
                                <td class="px-6 py-4">
                                    <div class="text-sm text-gray-900">{{ $incident->title ?? $incident->incident_type }}</div>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="text-sm text-gray-500">{{ $incident->incident_date->format('Y-m-d') }}</div>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="text-sm text-gray-500">{{ $incident->department?->name ?? 'N/A' }}</div>
                                </td>
                            </tr>
                        @empty
                            <tr>
