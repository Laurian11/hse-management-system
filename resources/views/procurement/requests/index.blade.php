...
<div class="bg-white border-b border-gray-300">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex justify-between items-center py-4">
            <div>
                <h1 class="text-2xl font-bold text-black">Procurement Requests</h1>
                <p class="text-sm text-gray-500 mt-1">Procurement requests & approvals</p>
            </div>
            <div>
                <a href="{{ route('procurement.dashboard') }}" class="text-[#0066CC] hover:underline mr-4">Dashboard</a>
                <a href="{{ route('procurement.requests.create') }}" class="bg-[#0066CC] text-white px-4 py-2 border border-[#0066CC] hover:bg-[#0052A3]">
    ...
    <div class="bg-white border border-gray-300 p-4 mb-6">
        <form method="GET" class="grid grid-cols-1 md:grid-cols-4 gap-4">
            ...
            <div class="flex space-x-2">
                ...
                <a href="{{ route('procurement.requests.index') }}" class="px-4 py-2 border border-gray-300 hover:bg-[#F5F5F5]">Reset</a>
            </div>
        </form>
    </div>
        ...
        <table class="min-w-full divide-y divide-gray-300">
            <thead class="bg-[#F5F5F5]">
                <tr>
                    ...
                    <th class="px-6 py-3 text-left text-xs font-medium text-black uppercase">Requested By</th>
            ...
            <tbody class="bg-white divide-y divide-gray-300">
                @forelse($requests as $request)
                    <tr class="hover:bg-[#F5F5F5]">
                        <td class="px-6 py-4 text-sm font-medium text-black">{{ $request->reference_number }}</td>
                        <td class="px-6 py-4 text-sm text-black">{{ $request->item_name }}</td>
                        <td class="px-6 py-4 text-sm text-black">{{ $request->quantity }} {{ $request->unit ?? '' }}</td>
                        <td class="px-6 py-4 text-sm text-black">{{ $request->requestedBy->name ?? 'N/A' }}</td>
                        <td class="px-6 py-4">
                            <span class="px-2 py-1 text-xs font-semibold border border-gray-300
                                ...
                                {{ in_array($request->status, ['submitted', 'under_review']) ? 'bg-[#F5F5F5] text-[#FF9900] border-[#FF9900]' : '' }}">
                                {{ ucfirst(str_replace('_', ' ', $request->status)) }}
                            </span>
                        </td>
                        <td class="px-6 py-4 text-right text-sm">
                            <a href="{{ route('procurement.requests.show', $request) }}" class="text-[#0066CC] hover:underline mr-3">View</a>
                            <a href="{{ route('procurement.requests.edit', $request) }}" class="text-[#0066CC] hover:underline">Edit</a>
                        </td>
                    </tr>
                @empty
