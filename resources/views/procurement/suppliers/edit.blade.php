    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex justify-between items-center py-4">
            <div class="flex items-center space-x-3">
                <a href="{{ route('procurement.suppliers.show', $supplier) }}" class="text-gray-500 hover:text-black">
                    <i class="fas fa-arrow-left mr-2"></i>Back
                </a>
                <h1 class="text-2xl font-bold text-black">Edit Supplier</h1>
            </div>
        </div>
    </div>
</div>
...
<div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
    <form action="{{ route('procurement.suppliers.update', $supplier) }}" method="POST" class="space-y-6">
        @csrf
        @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <textarea id="address" name="address" rows="2"
                              class="w-full px-3 py-2 border border-gray-300 focus:border-[#0066CC] focus:ring-1 focus:ring-[#0066CC]">{{ old('address', $supplier->address) }}</textarea>
                </div>
                ...
                <div>
                    ...
                    <select id="supplier_type" name="supplier_type" required
                        ...
                        <option value="materials" {{ old('supplier_type', $supplier->supplier_type) == 'materials' ? 'selected' : '' }}>Materials</option>
                        <option value="waste_disposal" {{ old('supplier_type', $supplier->supplier_type) == 'waste_disposal' ? 'selected' : '' }}>Waste Disposal</option>
                        <option value="medical" {{ old('supplier_type', $supplier->supplier_type) == 'medical' ? 'selected' : '' }}>Medical</option>
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea id="notes" name="notes" rows="3"
                              class="w-full px-3 py-2 border border-gray-300 focus:border-[#0066CC] focus:ring-1 focus:ring-[#0066CC]">{{ old('notes', $supplier->notes) }}</textarea>
                </div>
            </div>
        </div>

        <div class="flex justify-end space-x-3">
            <a href="{{ route('procurement.suppliers.show', $supplier) }}" class="px-6 py-2 border border-gray-300 hover:bg-[#F5F5F5]">Cancel</a>
