...
<div class="bg-white border-b border-gray-300">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex justify-between items-center py-4">
            <div>
                <h1 class="text-2xl font-bold text-black">Document Versions</h1>
                <p class="text-sm text-gray-500 mt-1">Version control and approval workflow</p>
            </div>
            <div>
                <a href="{{ route('documents.dashboard') }}" class="text-[#0066CC] hover:underline mr-4">Dashboard</a>
                <a href="{{ route('documents.versions.create') }}" class="bg-[#0066CC] text-white px-4 py-2 border border-[#0066CC] hover:bg-[#0052A3]">
...
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
    <div class="bg-white border border-gray-300 p-4 mb-6">
        <form method="GET" class="grid grid-cols-1 md:grid-cols-4 gap-4">
            ...
            <select name="document_id" class="px-3 py-2 border border-gray-300 focus:border-[#0066CC] focus:ring-1 focus:ring-[#0066CC]">
            ...
            <div class="flex space-x-2">
                ...
                <a href="{{ route('documents.versions.index') }}" class="px-4 py-2 border border-gray-300 hover:bg-[#F5F5F5]">Reset</a>
            </div>
        </form>
    </div>
    ...
    <div class="bg-white border border-gray-300 overflow-hidden">
        <table class="min-w-full divide-y divide-gray-300">
            <thead class="bg-[#F5F5F5]">
                <tr>
                    <th class="px-6 py-3 text-left text-xs font-medium text-black uppercase">Document</th>
                    <th class="px-6 py-3 text-left text-xs font-medium text-black uppercase">Version</th>
                    ...
                    <th class="px-6 py-3 text-left text-xs font-medium text-black uppercase">Created Date</th>
            ...
            <tbody class="bg-white divide-y divide-gray-300">
                @forelse($versions as $version)
                    <tr class="hover:bg-[#F5F5F5]">
                        <td class="px-6 py-4 text-sm text-black">{{ $version->document->title ?? 'N/A' }}</td>
                        ...
                        <td class="px-6 py-4 text-sm text-black">{{ $version->creator->name ?? 'N/A' }}</td>
                        <td class="px-6 py-4 text-sm text-black">{{ $version->created_at->format('M d, Y') }}</td>
                        <td class="px-6 py-4 text-right text-sm">
                            <a href="{{ route('documents.versions.show', $version) }}" class="text-[#0066CC] hover:underline mr-3">View</a>
                            <a href="{{ route('documents.versions.edit', $version) }}" class="text-[#0066CC] hover:underline">Edit</a>
                        </td>
                    </tr>
                @empty
                    <tr><td colspan="6" class="px-6 py-4 text-center text-gray-500">No versions found.</td></tr>
                @endforelse
            </tbody>
        </table>
    </div>
